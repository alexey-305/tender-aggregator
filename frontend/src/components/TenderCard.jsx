...
import { FileText, MapPin, Clock, Download, ExternalLink, MessageCircle, Tag } from "lucide-react";
import api from "../api/client";

export default function TenderCard({ tender, onMarksChange, currentMarks }) {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (marksRef.current && !marksRef.current.contains(e.target)) {
        setShowMarks(false);
      }
    };
    if (showMarks) {
      ...
      <div className="p-6">
        ...
        {localMarks.length > 0 && (
          <div className="flex gap-1.5 mb-4 flex-wrap">
            {localMarks.map((mark) => (
              <span key={mark.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium" style={{ backgroundColor: mark.color + "20", color: mark.color }}>
                {mark.name}
                <button onClick={() => handleToggleMark(mark)} className="hover:opacity-70">&times;</button>
              </span>
            ))}
          </div>
        )}
