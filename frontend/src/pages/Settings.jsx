  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] p-8">
      <div className="max-w-3xl mx-auto">
        <button onClick={() => window.location.href = "/"} className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors mb-12"><ArrowLeft size={18} /> Назад</button>
        <h1 className="text-2xl font-bold text-[var(--text-primary)] mb-10">Настройки</h1>
        <div className="flex gap-3 mb-10">
          <button onClick={() => { setTab("keys"); setShowForm(false); }} className={"px-5 py-2.5 rounded-lg text-sm font-medium " + (tab === "keys" ? "bg-[var(--accent)] text-[var(--text-primary)]" : "bg-[var(--bg-tertiary)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]")}>Шаблоны</button>
          <button onClick={() => { setTab("marks"); setShowForm(false); }} className={"px-5 py-2.5 rounded-lg text-sm font-medium " + (tab === "marks" ? "bg-[var(--accent)] text-[var(--text-primary)]" : "bg-[var(--bg-tertiary)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]")}>Метки</button>
        </div>

        {!showForm && (
          <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-5 py-3 rounded-lg bg-[var(--accent)] text-[var(--text-primary)] text-sm font-medium hover:bg-[var(--accent-hover)] transition-colors mb-10">
        ...
        {showForm && tab === "keys" && (
          <div className="bg-[var(--bg-secondary)] p-6 rounded-xl border border-[var(--border)] mb-10 flex flex-col gap-6">
            <h2 className="text-lg font-bold text-[var(--text-primary)]">{editId ? "Редактирование шаблона" : "Новый шаблон"}</h2>

            <input type="text" placeholder="Название шаблона" value={name} onChange={(e) => setName(e.target.value)} className="w-full p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--accent)]" />
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input type="number" placeholder="От" value={priceFrom} onChange={(e) => setPriceFrom(e.target.value)} className="w-full p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--accent)]" />
                <span className="text-[var(--text-secondary)]">—</span>
                <input type="number" placeholder="До" value={priceTo} onChange={(e) => setPriceTo(e.target.value)} className="w-full p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--accent)]" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input type="number" placeholder="От" value={securityAppFrom} onChange={(e) => setSecurityAppFrom(e.target.value)} className="w-full p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--accent)]" />
                <span className="text-[var(--text-secondary)]">—</span>
                <input type="number" placeholder="До" value={securityAppTo} onChange={(e) => setSecurityAppTo(e.target.value)} className="w-full p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--accent)]" />
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input type="number" placeholder="От" value={securityContractFrom} onChange={(e) => setSecurityContractFrom(e.target.value)} className="w-full p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--accent)]" />
                <span className="text-[var(--text-secondary)]">—</span>
                <input type="number" placeholder="До" value={securityContractTo} onChange={(e) => setSecurityContractTo(e.target.value)} className="w-full p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--accent)]" />
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-[var(--border)]">
              <button onClick={handleCreate} className="px-6 py-2.5 rounded-lg bg-[var(--accent)] text-[var(--text-primary)] text-sm font-medium hover:bg-[var(--accent-hover)]">Сохранить и обновить результаты</button>
              <button onClick={() => { setShowForm(false); resetForm(); }} className="px-6 py-2.5 rounded-lg bg-[var(--bg-tertiary)] text-[var(--text-secondary)] text-sm hover:text-[var(--text-primary)]">Отменить</button>
        ...
        {showForm && tab === "marks" && (
          <div className="bg-[var(--bg-secondary)] p-6 rounded-xl border border-[var(--border)] mb-10 flex flex-col gap-5">
            <h2 className="text-lg font-bold text-[var(--text-primary)]">{editId ? "Редактирование метки" : "Новая метка"}</h2>
            <input type="text" placeholder="Название метки" value={name} onChange={(e) => setName(e.target.value)} className="w-full p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--accent)]" />
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-[var(--border)]">
              <button onClick={handleCreate} className="px-6 py-2.5 rounded-lg bg-[var(--accent)] text-[var(--text-primary)] text-sm font-medium hover:bg-[var(--accent-hover)]">Сохранить</button>
              <button onClick={() => { setShowForm(false); resetForm(); }} className="px-6 py-2.5 rounded-lg bg-[var(--bg-tertiary)] text-[var(--text-secondary)] text-sm hover:text-[var(--text-primary)]">Отменить</button>
        ...
        <div className="flex flex-col gap-3">{(tab === "keys" ? keys : marks).map((item) => (
          <div key={item.id} onClick={() => handleEdit(item)} className="flex items-center justify-between p-4 bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] hover:border-[var(--accent)]/50 transition-colors cursor-pointer">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-3">{tab === "marks" && <div className="w-5 h-5 rounded flex-shrink-0" style={{ backgroundColor: item.color }} />}<span className="text-[var(--text-primary)] font-medium">{item.name}</span></div>
      ...
      {/* Модальные окна */}
      <Modal isOpen={showRegions} onClose={() => setShowRegions(false)} title="Регион поставки" footer={<><button onClick={() => setShowRegions(false)} className="px-5 py-2 rounded-lg bg-[var(--accent)] text-[var(--text-primary)] text-sm font-medium hover:bg-[var(--accent-hover)]">Выбрать</button><button onClick={() => { setRegions([]); setShowRegions(false); }} className="px-5 py-2 rounded-lg bg-[var(--bg-tertiary)] text-[var(--text-secondary)] text-sm hover:text-[var(--text-primary)]">Сбросить</button></>}>
        ...
        <div className="flex flex-col gap-3">
          {ADVANTAGES.map((adv) => (
            <div key={adv} className="flex items-center gap-3">
              <span className="text-sm text-[var(--text-primary)] w-64">{adv}</span>
              <div className="flex gap-1">
                {["Не учитывать","Добавить","Исключить"].map((mode) => (
                  <button key={mode} onClick={() => setAdvantages(prev => ({ ...prev, [adv]: prev[adv] === mode ? null : mode }))} className={"px-3 py-1 rounded text-xs " + (advantages[adv] === mode ? "bg-[var(--accent)] text-[var(--text-primary)]" : "bg-[var(--bg-tertiary)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]")}>{mode}</button>
                ))}
              </div>
            </div>
          ))}
