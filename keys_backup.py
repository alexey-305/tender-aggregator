...
async def list_keys(db: AsyncSession = Depends(get_db)):
    ...
    return [{"id": str(k.id), "name": k.name, "keywords": k.keywords, "region": k.region, "law": k.law, "price_from": k.price_from, "price_to": k.price_to, "platforms": k.platforms, "methods": k.methods, "count": 0} for k in keys]

@router.post("", response_model=KeyOut)
async def create_key(body: KeyCreate, db: AsyncSession = Depends(get_db)):
    key = SearchKey(name=body.name, keywords=body.keywords, region=body.region, law=body.law, price_from=body.price_from, price_to=body.price_to, platforms=body.platforms, methods=body.methods)
    db.add(key); await db.commit(); await db.refresh(key)
    return {"id": str(key.id), "name": key.name, "keywords": key.keywords, "region": key.region, "law": key.law, "price_from": key.price_from, "price_to": key.price_to, "platforms": key.platforms, "methods": key.methods, "count": 0}
...
async def update_key(key_id: str, body: KeyCreate, db: AsyncSession = Depends(get_db)):
    ...
    key.platforms = body.platforms; key.methods = body.methods
