import argparse, asyncio, io, logging, zipfile
...
async def _get_or_create_customer(session, inn, name):
...
async def _upsert_tender(session, parsed: ParsedNotification, region: str, law: str = "44"):
    customer = await _get_or_create_customer(session, parsed.customer_inn, parsed.customer_name)
    source = TenderSource.EIS_44FZ if law == "44" else TenderSource.EIS_223FZ
    law_enum = ProcurementLaw.FZ_44 if law == "44" else ProcurementLaw.FZ_223
    ...
    stmt = stmt.on_conflict_do_update(constraint="uq_tender_source_external_id", set_=update_cols)
    await session.execute(stmt)

async def sync_region_date(region: str, exact_date: date, document_types=None, token=None, subsystem_type="PRIZ", law_type="44"):
    ...
    async with async_session_maker() as session:
        for document_type in document_types:
            stats["requested"] += 1
            try:
                archive_ref = client.request_archive(org_region=region, exact_date=exact_date, document_type=document_type, subsystem_type=subsystem_type)
